...
**待决问题**：
- [ ] 默认排序：目前 code 字母序，要不要改成评价数 desc？
- [x] ~~"申请新课"功能~~ → 已实现（顶部「添加课程」按钮 + CourseSubmitDialog）
- [x] ~~课程卡片评价数怎么取~~ → 已实现（getCourses 联表聚合，等同课组合并）
- [x] ~~Department 筛选~~ → 分类体系已重构为 Major / Minor / Core / GE，无 Department 维度
...
- [x] ~~semester 覆盖范围~~ → 年份（今年 ±5 内 6 个选项）+ 季节两个 dropdown
- [x] ~~中英 tab 还是双栏~~ → 两个 Textarea 同时显示
...
- [x] ~~已删除评价怎么展示~~ → 混在一起标灰 + 「显示已删除」开关（默认开）
...
|------|------|------|
| 1. `/login` | ✅ | ✅ |
| 2. `/` 筛选浏览 | ✅ | ✅（筛选 / 搜索 / 加载更多 / 评价数角标 / 添加课程） |
| 3. `/courses/[id]` | ✅ | ✅（等同课聚合 / 教授筛选 / 排序 / 写·编·删·恢复 / 点赞点踩） |
| 4. `/profile` | ✅ | ✅（匿名 ID / 统计 / 排序 / 已删除开关） |
